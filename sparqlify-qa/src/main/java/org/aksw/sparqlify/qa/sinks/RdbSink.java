...
public class RdbSink implements MeasureDataSink {
	...
	private void initDb() throws SQLException {
		...
		// quad pattern table
		conn.createStatement().executeUpdate(
				"CREATE TABLE IF NOT EXISTS " + quadPatternTbl + " (" +
					"id bigint PRIMARY KEY, " +
					"graph varchar(300), " +
					"subject varchar(300), " +
					"predicate varchar(300), " +
		...
		// measure datum  --  quad pattern
		conn.createStatement().executeUpdate(
				"CREATE TABLE IF NOT EXISTS " + md2quadPatternTbl + " (" +
					"measure_datum_id bigint REFERENCES " + measureDatumTbl + "(id), " +
					"quad_pattern_id bigint REFERENCES " + quadPatternTbl + "(id), " +
					"assessment_id bigint, " +
					"PRIMARY KEY(measure_datum_id, quad_pattern_id, assessment_id)" +
				");");
		...
		conn.createStatement().executeUpdate(
				"CREATE TABLE IF NOT EXISTS " + trpl2quadTbl + " (" +
					...
					"quad_pattern_id bigint REFERENCES " + quadPatternTbl + "(id), " +
					"assessment_id bigint, " +
					"PRIMARY KEY (triple_id, quad_pattern_id, assessment_id)" +
				");");
		...
		// view definition  --  quad pattern
		conn.createStatement().executeUpdate(
				"CREATE TABLE IF NOT EXISTS " + vd2quadTbl + " (" +
					"view_definition_id bigint REFERENCES view_definition(id), " +
					"quad_pattern_id bigint REFERENCES quad_pattern(id), " +
					"assessment_id bigint, " +
					"PRIMARY KEY(view_definition_id, quad_pattern_id, assessment_id)" +
				");");
	...
	private void writeMappingQuadMeasureDatum(MappingQuadMeasureDatum datum)
		...
		for (Pair<Quad, ViewDefinition> viewQuad : datum.getQuadViewDefs()) {
			...
			// quadPattern may be null in case of the property completeness metric
			Quad quadPattern = viewQuad.first;
			if (quadPattern != null) {
				long quadPatternId = writeQuadPattern(quadPattern);
				// write n:m entries
				writeMeasureDatum2quadPattern(measureDatumId, quadPatternId);
	...
	private void writeTriplesMeasureDatum(TriplesMeasureDatum datum)
		...
		for (Pair<Triple, Set<ViewQuad<ViewDefinition>>> tripleViewQuads : datum.getPinpoinInfos()) {
			...
			for (ViewQuad<ViewDefinition> viewQuad : viewQuads) {
				// write quad
				Quad quadPattern = viewQuad.getQuad();
				long quadId = writeQuadPattern(quadPattern);
	...
	private long writeQuadPattern(Quad quadPattern)
			throws SQLException {
		
		long quadPatternId;
		
		String quadPatternGraph = quadPattern.getGraph().getURI();

		Node subject = quadPattern.getSubject();
		String quadPatternSubject = getNodeStrRepr(subject);
		
		Node predicate = quadPattern.getPredicate();
		String quadPatternPredicate = getNodeStrRepr(predicate);
		
		// get object string representation
		Node object = quadPattern.getObject();
		String quadPatternObject = getNodeStrRepr(object);
		
		
		// check if already exists
		PreparedStatement qpQuery = conn.prepareStatement(
				"SELECT id FROM " + quadPatternTbl + " " +
				"WHERE graph=? AND subject=? AND predicate=? AND object=?;");
		qpQuery.setString(1, quadPatternGraph);
		qpQuery.setString(2, quadPatternSubject);
		qpQuery.setString(3, quadPatternPredicate);
		qpQuery.setString(4, quadPatternObject);
		ResultSet qpRes = qpQuery.executeQuery();
		
		if (!qpRes.next()) {
			// quad pattern does not exist in the database, yet
			quadPatternId = getNextId();
			PreparedStatement qpStmnt = conn.prepareStatement(
					"INSERT INTO " + quadPatternTbl + " VALUES (?, ?, ?, ?, ?);");
			qpStmnt.setLong(1, quadPatternId);
			qpStmnt.setString(2, quadPatternGraph);
			qpStmnt.setString(3, quadPatternSubject);
			qpStmnt.setString(4, quadPatternPredicate);
			qpStmnt.setString(5, quadPatternObject);
			qpStmnt.executeUpdate();
			
		} else {
			// quad pattern does exist
			quadPatternId = qpRes.getLong("id");
		}
		
		return quadPatternId;
	}
	...
	private void writeMeasureDatum2quadPattern(long measureDatumId,
			long quadPatternId) throws SQLException {
		
		// check if entry already exists
		PreparedStatement md2qpQStmnt = conn.prepareStatement(
				"SELECT measure_datum_id FROM " + md2quadPatternTbl + " " +
				"WHERE measure_datum_id=? AND quad_pattern_id=? AND assessment_id=?;");
		md2qpQStmnt.setLong(1, measureDatumId);
		md2qpQStmnt.setLong(2, quadPatternId);
		...
		if (!res.next()) {
			...
			PreparedStatement md2qpIStmnt = conn.prepareStatement(
					"INSERT INTO " + md2quadPatternTbl + " VALUES (?, ?, ?);");
			md2qpIStmnt.setLong(1, measureDatumId);
			md2qpIStmnt.setLong(2, quadPatternId);
	...
	private void writeTriple2quad(long tripleId, long quadId) throws SQLException {
		...
		PreparedStatement trpl2qudQStmnt = conn.prepareStatement(
				...
				"WHERE triple_id=? AND quad_pattern_id=? AND assessment_id=?;");
	...
	private void writeViewDef2quadPattern(long viewDefId, long quadPatternId)
			throws SQLException {
		
		// check if entry already exists
		PreparedStatement vd2qpQStmnt = conn.prepareStatement(
				"SELECT view_definition_id FROM " + vd2quadTbl + " " +
				"WHERE view_definition_id=? AND quad_pattern_id=? " +
					"AND assessment_id=?");
		vd2qpQStmnt.setLong(1, viewDefId);
		vd2qpQStmnt.setLong(2, quadPatternId);
		...
		if (!res.next()) {
			...
			vd2qpIStmnt.setLong(2, quadPatternId);
